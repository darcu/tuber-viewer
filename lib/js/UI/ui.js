...
function createListItemContent(args) {
	var el;
	if (args.thumbUrl) {
		el = `<img src="${args.thumbUrl}">`;
	}

	el +=
		`<span class="listinfocontainer">${args.title} (${args.channel}) - ${args.duration}</span>`;
...
addSearch.addEventListener('keyup', function(e) {
	if (e.keyCode === 13) {
		CTRL.searchFor(e.target.value).then(function(results) {
			clearSearchList();

			for (var i = 0, n = results.length; i < n; i++) {
				addToSearchList(results[i]);
			}
		}, UTIL.err);
	}
});
...
EV.addListener('AUTHDONE', function(e) {
	...
	if (ENV.USER.authenticated) {
		signinButton.innerHTML = '<i class="fa fa-sign-out"></i>';
		signinButton.onclick = function() {
			CTRL.signOut();
		};
	} else {
		signinButton.innerHTML = '<i class="fa fa-sign-in"></i>';
		signinButton.onclick = function() {
			CTRL.signIn();
		};
	}
